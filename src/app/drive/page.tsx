  ...
  type RouteData = { coordinates: [number, number][]; steps: { instruction: string; distance: number }[]; hazards: Hazard[]; distance: number; duration: number; avoidedHazards?: number; tookAlternateRoute?: boolean };
  ...
  export default function DrivePage() {
    ...
    // Throttles map.easeTo() so rapid GPS ticks can't interrupt each other's
    // in-flight camera animation (that fighting was causing the jerky feel).
    const lastCameraMoveRef = useRef(0);
    const CAMERA_MOVE_MS = 900; // must be >= the easeTo duration below (700ms)
    ...
    function speak(text: string) {
      ...
      try {
        if ("speechSynthesis" in window) {
          // Cancel anything still queued/speaking first — without this, a
          // second alert waits for the first to finish and plays after
          // you've already passed the hazard.
    ...
    function startWatch(d: RouteData) {
      ...
      // Clear any watcher already running before starting a new one — otherwise
      // re-routing/restarting stacks a second live GPS watcher on top of the
      // first, doubling voice alerts and fighting the camera.
      if (watchId.current != null) navigator.geolocation.clearWatch(watchId.current);
      watchId.current = navigator.geolocation.watchPosition((p) => {
        ...
        // Full pitched easeTo only once per CAMERA_MOVE_MS; cheaper jumpTo in
        // between so a fast GPS tick can never interrupt an in-flight easeTo
        // animation (that fighting was causing the jerky camera).
        const now = Date.now();
        if (now - lastCameraMoveRef.current >= CAMERA_MOVE_MS) {
          lastCameraMoveRef.current = now;
          mapRef.current?.easeTo({ center: [cur.lng, cur.lat], zoom: 17, pitch: 60, duration: 700 });
        } else {
          mapRef.current?.jumpTo({ center: [cur.lng, cur.lat] });
        }
        const prefs = prefsRef.current;
    ...
    async function start() {
      ...
      try {
        ...
        if (d.tookAlternateRoute && d.avoidedHazards) {
          speak(`Found a safer route. Avoided ${d.avoidedHazards} hazard${d.avoidedHazards === 1 ? "" : "s"} ahead.`);
        } else if (d.hazards?.length) {
          speak(`${d.hazards.length} hazard${d.hazards.length === 1 ? "" : "s"} on your route. Drive safe.`);
        } else {
          speak("Route is clear. Drive safe.");
        }
        startWatch(d);
          ...
          <>
            ...
            <div className="absolute inset-x-0 bottom-0 z-20 mx-auto max-w-md p-4">
              <div className="rounded-2xl border p-4" style={{ background: "var(--surface)", borderColor: "var(--border)", boxShadow: "var(--shadow-lg)" }}>
                ...
                ) : route?.tookAlternateRoute && route.avoidedHazards ? (
                  <div className="flex items-center justify-center gap-2 text-sm font-semibold" style={{ color: "var(--brand-strong)" }}>
                    <IconCheck size={16} /> Took a safer route · avoided {route.avoidedHazards} hazard{route.avoidedHazards === 1 ? "" : "s"}
                  </div>
                ) : (
